...
class Wheel : public Composite, public steppable{
  ...
  Line *spoke[10];
public:
  ~Wheel(){delete rim; for(int i=0; i<10; i++) delete spoke[i];}
  Wheel(double x, double y, double rr, double vxx, double vyy,
    Composite* owner=NULL) : Composite(x,y,owner){
    omega=2*PI/72;
    r = rr;
    vx = vxx;
    vy = vyy;
    ...
    for(int i=0; i<10; i++){
      spoke[i] = new Line(0, 0, r*cos(i*PI/5), r*sin(i*PI/5), this);
    }
  }
  virtual bool step(){
      move(vx,vy);
    ...
    for(int phi=0; phi<360; phi += 18){
      ...
      Wheel* w =new Wheel(getX()+r*0.95*cos(phirad),
                  getY()+r*0.95*sin(phirad),
                  3*randuv(1,3),
...
void chakra(){
  ...
  Wheel *w = new Wheel(500,screenh/2,
               75,5,3);
  controller::stbls.push(w);
  repeat(100){
    controller::process();
